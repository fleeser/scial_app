...
class TestEndpoint extends Endpoint {
  ...
  Future<void> test(Session session) async {
    ...
    //FriendRequest fri = FriendRequest(sender: 2, receiver: 1, status: FriendRequestStatus.pending, created: DateTime.now(), text: "Ey wir kennen uns doch oder nicht?");
    //await FriendRequest.insert(session, fri);
    //await Notification.insert(session, Notification(created: DateTime.now(), type: NotificationType.friendRequestCreated, read: false, receiver: 1, ref: fri.id!));
