/* AUTOMATICALLY GENERATED CODE DO NOT MODIFY */
/*   To generate run: "serverpod generate"    */

// ignore_for_file: library_private_types_in_public_api
// ignore_for_file: public_member_api_docs
// ignore_for_file: implementation_imports

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'package:serverpod/serverpod.dart' as _i1;
import '../../protocol.dart' as _i2;

class DiscoverReadResponse extends _i1.SerializableEntity {
  DiscoverReadResponse({
    required this.success,
    this.code,
    this.events,
  });

  factory DiscoverReadResponse.fromJson(
    Map<String, dynamic> jsonSerialization,
    _i1.SerializationManager serializationManager,
  ) {
    return DiscoverReadResponse(
      success:
          serializationManager.deserialize<bool>(jsonSerialization['success']),
      code: serializationManager.deserialize<_i2.DiscoverReadResponseCode?>(
          jsonSerialization['code']),
      events: serializationManager
          .deserialize<List<_i2.PublicEvent>?>(jsonSerialization['events']),
    );
  }

  bool success;

  _i2.DiscoverReadResponseCode? code;

  List<_i2.PublicEvent>? events;

  @override
  Map<String, dynamic> toJson() {
    return {
      'success': success,
      'code': code,
      'events': events,
    };
  }

  @override
  Map<String, dynamic> allToJson() {
    return {
      'success': success,
      'code': code,
      'events': events,
    };
  }
}
